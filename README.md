...
上記のいずれか1つでも成立したレースを Discord に送信します。3号艇または4号艇の `捲り` は過去集計で相対的に良かったため、1号艇の `捲られ` が 8%以上なら期待値追加候補として拾います。加えて `今節成績` タブの `ST順位` で `3号艇 >= 4.0` かつ `4号艇 <= 2.7` の場合は、`4まくり高期待値` として拾い、4頭・3号艇基本消しの買い目を組みます。通知には過去サンプルの回収率を目安にした `期待値目安` と `EV指数` を出します。レースの並び順には内部で優先度を使いますが、通知文面は簡潔に保ちます。Discord Webhook には下書き機能がないため、朝通知は `[DRAFT]` タイトル付きの `embed` として送信します。
