...
const DEFAULT_FINAL_TARGET_SYNTHETIC_ODDS = 8.5;
const DEFAULT_HIGH_ODDS_TICKET_MIN_ODDS = 300;
const DEFAULT_HIGH_ODDS_SYNTHETIC_MIN_ODDS = 25;
const HIGH_ODDS_ALERT_MAX_TICKETS = 8;
const FINAL_INFO_LABEL_ALIASES = new Map([
  ["displayTopCount", ["1位回数"]],
  ["displayTime", ["展示タイム", "展示"]],
  ["st", ["ST"]],
  ["exhibitionCourse", ["展示進入", "進入", "コース"]]
]);
const SEASON_INFO_LABEL_ALIASES = new Map([
  ["seasonStRank", ["今節ST順位", "節間ST順位", "ST順位"]],
  ["seasonRank", ["今節順位", "順位"]],
  ["motor2Rate", ["モーター2連対率", "モーター2連率"]],
  ["frame2Rate", ["枠別2連対率", "枠別連対率", "艇番別2連対率", "艇番2連対率", "コース別2連対率", "2連対率"]],
  ["seasonAverageSt", ["今節平均ST", "平均ST", "ST"]]
]);
...
function parseEnvPositiveNumber(value, label, defaultValue) {
  if (!value) {
    return defaultValue;
  }
...
function loadConfig() {
  ...
  return {
    ...
    hiduke,
    dryRun,
    targetSyntheticOdds: parseEnvPositiveNumber(
      process.env.FINAL_TARGET_SYNTHETIC_ODDS,
      "FINAL_TARGET_SYNTHETIC_ODDS",
      DEFAULT_FINAL_TARGET_SYNTHETIC_ODDS
    ),
    highOddsTicketMinOdds: parseEnvPositiveNumber(
      process.env.FINAL_HIGH_ODDS_TICKET_MIN_ODDS,
      "FINAL_HIGH_ODDS_TICKET_MIN_ODDS",
      DEFAULT_HIGH_ODDS_TICKET_MIN_ODDS
    ),
    highOddsSyntheticMinOdds: parseEnvPositiveNumber(
      process.env.FINAL_HIGH_ODDS_SYNTHETIC_MIN_ODDS,
      "FINAL_HIGH_ODDS_SYNTHETIC_MIN_ODDS",
      DEFAULT_HIGH_ODDS_SYNTHETIC_MIN_ODDS
    )
  };
}
...
async function extractLabeledBoatRows(page, labelAliases) {
  const extracted = await page.evaluate((entries) => {
    ...
    const aliasCandidates = entries
      .flatMap(([canonical, candidateAliases]) => candidateAliases.map((alias) => ({
        canonical,
...
function buildBoatMetrics(snapshot) {
  ...
  return Array.from({ length: 6 }, (_, index) => {
    ...
    return {
      ...
      exhibitionCourse: getArrayValue(snapshot.exhibitionCourseRow, index, parseInteger),
      ...
      seasonAverageSt: getArrayValue(snapshot.seasonAverageStRow, index, parseStCell),
      seasonStRank: getArrayValue(snapshot.seasonStRankRow, index, parseNumber),
      st
    };
  });
}
...
function computeSeasonStScore(metric) {
  const rank = metric.seasonStRank;
  if (!Number.isInteger(rank)) {
    return 0;
  }
...
function computeSeasonStShiftBonus(candidateMetric, mainMetric) {
  if (!candidateMetric || !mainMetric) {
    return 0;
  }

  const candidateRank = candidateMetric.seasonStRank;
  const mainRank = mainMetric.seasonStRank;
  if (typeof candidateRank !== "number" || typeof mainRank !== "number") {
    return 0;
  }

  if (candidateRank <= 2.5 && mainRank >= 4) {
    return 1.5;
  }
  if (candidateRank < mainRank) {
    return 0.5;
  }
  if (candidateRank >= 5 && mainRank <= 3) {
    return -1;
  }
...
function computeExhibitionCourseBonus(metric) {
  const course = metric.exhibitionCourse;
  if (!Number.isInteger(course)) {
    return 0;
  }

  if (course < metric.boat) {
    return 1.5;
  }
  if (course > metric.boat) {
    return -1;
...
function evaluatePrimaryFocus(mainPrediction) {
  const score = typeof mainPrediction?.score === "number" ? mainPrediction.score : null;

  if (mainPrediction?.type !== "捲り") {
    return {
      allow: false,
      note: "最終送信対象外: 捲り以外"
    };
  ...
  if (mainPrediction.boat === 4) {
    if (score === null) {
      return {
        allow: false,
        note: "最終送信対象外: 4頭捲りスコア不明"
      };
    }

    return score < 12
      ? { allow: true, note: null }
      : { allow: false, note: "最終送信対象外: 4頭捲り score12以上" };
  }

  if (mainPrediction.boat === 3) {
    if (score === null) {
      return {
        allow: false,
        note: "最終送信対象外: 3頭捲りスコア不明"
      };
    }

    return score < 15
      ? { allow: true, note: null }
      : { allow: false, note: "最終送信対象外: 3頭捲り score15以上" };
  }

  return {
    allow: false,
    note: "最終送信対象外: 3頭/4頭捲り以外"
  };
}
...
function buildExtraHeadCandidates(mainPrediction, metrics) {
  ...
  const mainMetric = metrics[mainPrediction.boat - 1] || null;
  ...
  return candidatePool
    .map((boat) => {
      ...
      const score = computeSeasonRankScore(metric.seasonRank) +
        ...
        computeSeasonStScore(metric) +
        computeSeasonStShiftBonus(metric, mainMetric) +
        computeExhibitionCourseBonus(metric) +
...
function buildTicketSelection(mainPrediction, extraHeads, oddsByCombination, targetSyntheticOdds) {
  ...
  const selected = [];
  const ticketEntriesByPriority = [...deduped.values()].sort((left, right) => {
  ...
  for (const entry of ticketEntriesByPriority) {
    if (selected.length >= FINAL_MAX_TICKETS) {
      break;
    }

    const candidateTickets = [...selected.map((item) => item.ticket), entry.ticket];
    const candidateSyntheticOdds = calculateSyntheticOdds(candidateTickets, oddsByCombination);
    if (typeof candidateSyntheticOdds === "number" && candidateSyntheticOdds >= targetSyntheticOdds) {
      selected.push(entry);
    }
  }
  ...
  if (selected.length === 0) {
    return {
      ...
      tickets: [],
      syntheticOdds: null,
      note: `合成オッズ${targetSyntheticOdds.toFixed(1)}倍以上で買い目を組めない`
    };
  }
  ...
  return {
    ...
    syntheticOdds: calculateSyntheticOdds(selected.map((item) => item.ticket), oddsByCombination)
  };
}

function buildDisplayTicketsFromMorning(plans) {
  if (!plans?.honmei || !plans?.ana) {
    return [];
  }

  const tickets = [];
  for (const plan of [plans.honmei, plans.ana]) {
...
function buildHighOddsCandidates(tickets, oddsByCombination, minOdds, maxTickets = HIGH_ODDS_ALERT_MAX_TICKETS) {
  return tickets
    .map((ticket) => ({
      ticket,
      odds: oddsByCombination.get(ticket) ?? null
    }))
    .filter((item) => typeof item.odds === "number" && item.odds >= minOdds)
    .sort((left, right) => right.odds - left.odds)
    .slice(0, maxTickets);
}

function buildHighOddsAlert(race, oddsByCombination, config) {
  const morningTickets = race.kaime?.status === "ok" ? buildDisplayTicketsFromMorning(race.kaime.plans) : [];
  const candidates = buildHighOddsCandidates(morningTickets, oddsByCombination, config.highOddsTicketMinOdds);
  const tickets = candidates.map((item) => item.ticket);
  const syntheticOdds = calculateSyntheticOdds(tickets, oddsByCombination);

  if (tickets.length === 0 || typeof syntheticOdds !== "number" || syntheticOdds < config.highOddsSyntheticMinOdds) {
    return null;
  }
  ...
  return {
    tickets,
    syntheticOdds,
    highOddsCandidates: candidates,
    note: `高オッズ買い目アラート: ${config.highOddsTicketMinOdds.toFixed(0)}倍以上 / 合成${syntheticOdds.toFixed(1)}倍`
  };
}
...
function formatHighOddsCandidateLines(candidates) {
  ...
  return [
    "高オッズ買い目:",
    ...candidates.map((item) => `- ${item.ticket} ${formatOdds(item.odds)}`)
  ];
}

function buildRaceBlock(race, remainingMinutes, decision, metrics, config) {
  ...
  const lines = [
    ...
    `合成オッズ: ${formatOdds(decision.syntheticOdds)} / 目標${formatOdds(config.targetSyntheticOdds)}`,
    `点数: ${decision.tickets.length}点`,
    mainMetric
      ? `主軸気配: 展示${mainMetric.displayTime ?? "-"} / ST ${formatSt(mainMetric.st.raw, mainMetric.st.value)} / 展示進入${mainMetric.exhibitionCourse ?? "-"} / モーター${formatPercent(mainMetric.motor2Rate)}`
      : null,
    typeof mainMetric?.seasonStRank === "number"
      ? `節間ST順位: ${mainMetric.seasonStRank.toFixed(2)} / 平均${formatSt(mainMetric.seasonAverageSt?.raw, mainMetric.seasonAverageSt?.value)}`
      : null,
    decision.note || null,
    ...formatHighOddsCandidateLines(decision.highOddsCandidates),
    "買い目:",
...
async function scrapeRaceSnapshot(context, race) {
  ...
  try {
    ...
    return {
      ...
      exhibitionCourseRow: finalInfoRows.get("exhibitionCourse") || null,
      ...
      seasonAverageStRow: seasonRows.get("seasonAverageSt") || null,
      seasonStRankRow: seasonRows.get("seasonStRank") || null
    };
  } finally {
...
async function processRace(context, race, config) {
  ...
  const highOddsAlert = buildHighOddsAlert(race, oddsByCombination, config);
  const primaryFocus = evaluatePrimaryFocus(mainPrediction);
  if (!primaryFocus.allow) {
    if (highOddsAlert) {
      const decision = {
        status: "send",
        note: highOddsAlert.note,
        mainHead: mainPrediction.boat,
        mainType: mainPrediction.type,
        extraHeads: [],
        tickets: highOddsAlert.tickets,
        syntheticOdds: highOddsAlert.syntheticOdds,
        highOddsCandidates: highOddsAlert.highOddsCandidates
      };
  ...
  const ticketSelection = buildTicketSelection(mainPrediction, extraHeads, oddsByCombination, config.targetSyntheticOdds);
  if (ticketSelection.status !== "send") {
    ...
    applyDecision(race, deadlineAtJst, remainingMinutes, {
      status: "skip",
      note: ticketSelection.note || "買い目を組めない",
      mainHead: mainPrediction.boat,
      mainType: mainPrediction.type,
      extraHeads: extraHeads.map((item) => item.boat),
      tickets: [],
  ...
  const decision = {
    ...
    highOddsCandidates: buildHighOddsCandidates(ticketSelection.tickets, oddsByCombination, config.highOddsTicketMinOdds)
  };
  applyDecision(race, deadlineAtJst, remainingMinutes, decision);
...
async function sendFinalJudgement(config, items) {
  ...
  const payloads = buildChunkedDiscordPayloads({
    ...
    summary: [
      `対象日: ${config.hiduke}`,
      `送信レース: ${items.length}`,
      `条件: 締切${FINAL_LOOKAHEAD_MINUTES}分以内 / 4頭捲り score<12 または 3頭捲り score<15 / 合成オッズ${config.targetSyntheticOdds.toFixed(1)}倍以上 / 上限${FINAL_MAX_TICKETS}点`,
      `高オッズ通知: 朝買い目内 ${config.highOddsTicketMinOdds.toFixed(0)}倍以上 / 合成${config.highOddsSyntheticMinOdds.toFixed(1)}倍以上`
    ].join("\n"),
    blocks: items.map((item) => buildRaceBlock(item.race, item.remainingMinutes, item.decision, item.metrics, config)),
    color: 15105570
  });
