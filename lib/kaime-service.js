...
import { buildDualKaimePlans, formatMorningValueProfile, getMorningValueProfile } from "./kaime.js";
...
function formatReason(reason) {
  ...
  const expandedNote = reason.selectionLevel === "value-expanded" ? " / 期待値追加" : "";
  return `${reason.type}: ${reason.boat}号艇 ${formatPercent(reason.attack)} > 1号艇${reason.defenseLabel} ${formatPercent(reason.defense)}${expandedNote}`;
}
...
export function buildKaimeTextResponse({
  ...
  const lines = [
    ...
    primary ? formatMorningValueProfile(getMorningValueProfile(primary)) : null,
...
export function buildKaimeRaceBlock({
  ...
  if (primary) {
    lines.push(formatMorningValueProfile(getMorningValueProfile(primary)));
