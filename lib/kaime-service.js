...
function formatTicketLines(tickets, oddsByCombination) {
  if (tickets.length === 0) {
    return ["- なし"];
  }

  return tickets.map((ticket) => `- ${ticket} ${formatOdds(oddsByCombination.get(ticket))}`);
}

function formatPlanLine(label, plan, oddsByCombination) {
  const statusLabel = plan.status === "buy" ? "買い" : ["skip", "no-match", "no-tickets", "no-rule"].includes(plan.status) ? "該当なし" : plan.status;
  const oddsLabel = plan.status === "buy" && plan.syntheticOdds !== null ? ` 合成${formatOdds(plan.syntheticOdds)}` : "";
...
export function buildKaimeTextResponse({
  ...
  if (sharedTickets && plans.ana.tickets.length > 0) {
    lines.push(
      `共通買い目: 穴 ${formatPlanStatus(plans.ana)}${plans.ana.syntheticOdds !== null ? ` ${formatOdds(plans.ana.syntheticOdds)}` : ""} / 本命 ${formatPlanStatus(plans.honmei)}${plans.honmei.syntheticOdds !== null ? ` ${formatOdds(plans.honmei.syntheticOdds)}` : ""}`
    );
    lines.push(...formatTicketLines(plans.ana.tickets, oddsByCombination));
  } else {
    lines.push(formatPlanLine(`穴狙い(合成${anaMinCombinedOdds}倍以上)`, plans.ana, oddsByCombination));
    lines.push(formatPlanLine(`本命(合成${honmeiMinCombinedOdds}〜${honmeiMaxCombinedOdds}倍)`, plans.honmei, oddsByCombination));
  }
...
export function buildKaimeRaceBlock({
  ...
  if (sharedTickets && plans.ana.tickets.length > 0) {
    ...
    lines.push(...formatTicketLines(plans.ana.tickets, oddsByCombination));
    lines.push(
      `本命(合成${honmeiMinCombinedOdds}〜${honmeiMaxCombinedOdds}倍): ${formatPlanStatus(plans.honmei)}${plans.honmei.status === "buy" && plans.honmei.syntheticOdds !== null ? ` / 合成${formatOdds(plans.honmei.syntheticOdds)}` : ""}`
    );
    lines.push(...formatTicketLines(plans.honmei.tickets, oddsByCombination));
  }
...
export async function queryKaimeForRace({
  ...
  if (honmeiMaxCombinedOdds < honmeiMinCombinedOdds) {
    throw new Error("honmeiMaxCombinedOdds must be >= honmeiMinCombinedOdds");
  }
