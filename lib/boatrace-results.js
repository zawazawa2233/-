...
function parseRowsToPayoutMap(rows) {
  ...
  while (index < rows.length) {
    ...
    const headerGroupWidth = blockVenues.length > 0 ? Math.max(1, Math.floor(headerRow.length / blockVenues.length)) : 1;
    index += 1;
    ...
    while (index < rows.length) {
      ...
      const groupWidth = Math.max(1, Math.floor(cells.length / blockVenues.length), headerGroupWidth);
