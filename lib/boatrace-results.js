...
function parseRaceChunk(lines, venueCount) {
  ...
  while (cursor < lines.length && entries.length < venueCount) {
    ...
    while (cursor < lines.length && guard < 8 && (!combination || payoutYen === null)) {
      const line = lines[cursor];
      if (!combination) {
        combination = parseCombination(line);
      }
      if (payoutYen === null) {
        payoutYen = parseYen(line);
      }
      cursor += 1;
      guard += 1;
    }
...
export async function fetchBoatraceDailyPayouts(hiduke) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    while (cursor < lines.length) {
      ...
      if (PLACE_NAME_TO_NO.has(nextLine)) {
        break;
      }
