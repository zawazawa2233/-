...
export function buildPickedRaceState({ hiduke, matchedResults, kaimeByRace = new Map() }) {
  const races = matchedResults.map((result) => ({
    ...
    deadlineTime: typeof result.deadlineTime === "string" ? result.deadlineTime : null,
    ...
    matchReasons: result.matches.map((match) => ({
      ...
      selectionLevel: typeof match.selectionLevel === "string" ? match.selectionLevel : null
    })),
...
function isValidMatchReason(reason) {
  return reason &&
    ...
    (reason.selectionLevel === undefined || reason.selectionLevel === null || typeof reason.selectionLevel === "string");
}
...
function isValidKaimePrediction(prediction) {
  return prediction === null || (
    ...
    (prediction.selectionLevel === undefined || prediction.selectionLevel === null || typeof prediction.selectionLevel === "string")
  );
}
...
export function validatePickedRaceState(parsed) {
  ...
  for (const race of parsed.races) {
    ...
    if (race.deadlineTime !== undefined && race.deadlineTime !== null && typeof race.deadlineTime !== "string") {
      throw new Error("Pick state race deadlineTime is invalid");
    }
  ...
  return {
    ...
    races: parsed.races.map((race) => ({
      ...race,
      deadlineTime: typeof race.deadlineTime === "string" ? race.deadlineTime : null,
