...
const MORNING_VALUE_PROFILES = new Map([
  ["捲り-3", {
    grade: "S",
    evIndex: 123,
    priority: 4,
  ...
  ["差し-2", {
    ...
    evIndex: 35,
    priority: 1,
    summary: "2差しは当たりやすいが安め"
  }]
]);
const DEFAULT_MORNING_VALUE_PROFILE = {
  grade: "D",
  evIndex: 20,
  priority: 0,
  summary: "参考候補"
...
function getReasonScore(reason) {
  if (reason?.selectionLevel === ST_RANK_4_WATCH_LEVEL) {
    return 30 + Math.max(0, reason.defense - reason.attack);
  }

  return reason.attack - reason.defense;
}

export function getMorningValueProfile(prediction) {
  if (!prediction) {
    return null;
  }

  if (prediction.selectionLevel === ST_RANK_4_WATCH_LEVEL) {
    return {
      grade: "WATCH",
      evIndex: 88,
      priority: 5,
      summary: "4まくり高期待値/3基本消し",
      sample: "節間ST順位条件"
    };
  }

  const profile = MORNING_VALUE_PROFILES.get(`${prediction.type}-${prediction.boat}`) || DEFAULT_MORNING_VALUE_PROFILE;
  return {
    ...profile,
    sample: "2026/3後半+4月序盤"
  };
}

export function formatMorningValueProfile(profile) {
  if (!profile) {
    return "期待値目安: -";
  }

  return `期待値目安: ${profile.grade} / EV指数${profile.evIndex} (${profile.summary})`;
}
...
function buildStRank4WatchPlan(primaryReason, planType) {
  const secondBoats = planType === "honmei" ? [1, 5] : [2, 6];
  const thirdBoats = [1, 2, 5, 6];
  const tickets = buildTickets(4, secondBoats, thirdBoats);

  return {
    status: tickets.length > 0 ? "buy" : "no-tickets",
    prediction: primaryReason,
    phases: [{
      key: `st-rank-4-watch-${planType}`,
      label: planType === "honmei" ? "4頭3消し 本線" : "4頭3消し 押さえ",
      tickets
    }],
    tickets,
    syntheticOdds: null,
    note: planType === "honmei"
      ? "節間ST順位から4頭まくり高期待値を固定 (3号艇は基本消し)"
      : "節間ST順位から4頭まくり高期待値の押さえ (3号艇は基本消し)"
  };
}
...
export function buildKaimePlan({
  ...
  if (primaryReason.selectionLevel === ST_RANK_4_WATCH_LEVEL) {
    return buildStRank4WatchPlan(primaryReason, planType);
  }
