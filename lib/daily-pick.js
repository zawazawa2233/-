...
function loadConfig() {
  ...
  const anaMinCombinedOdds = parseEnvInt(process.env.ANA_MIN_COMBINED_ODDS, "ANA_MIN_COMBINED_ODDS", 10, 1);
  const honmeiMinCombinedOdds = parseEnvInt(process.env.HONMEI_MIN_COMBINED_ODDS, "HONMEI_MIN_COMBINED_ODDS", 3, 1);
  const honmeiMaxCombinedOdds = parseEnvInt(process.env.HONMEI_MAX_COMBINED_ODDS, "HONMEI_MAX_COMBINED_ODDS", 5, 1);
  if (honmeiMaxCombinedOdds < honmeiMinCombinedOdds) {
    throw new Error("HONMEI_MAX_COMBINED_ODDS must be >= HONMEI_MIN_COMBINED_ODDS");
  }
  const minTicketOdds = parseEnvInt(process.env.MIN_TICKET_ODDS, "MIN_TICKET_ODDS", 0, 0);
...
async function buildKaimeSummaries(state, config) {
    ...
    while (true) {
      ...
      try {
        const result = await queryKaimeForRace({
          hiduke: state.hiduke,
          race,
          anaMinCombinedOdds: config.anaMinCombinedOdds,
          honmeiMinCombinedOdds: config.honmeiMinCombinedOdds,
          honmeiMaxCombinedOdds: config.honmeiMaxCombinedOdds,
          minTicketOdds: config.minTicketOdds
        });
        results.push({
          race,
          status: "ok",
  ...
  const anaBuyCount = okResults.filter((item) => item.plans.ana.status === "buy").length;
  const honmeiBuyCount = okResults.filter((item) => item.plans.honmei.status === "buy").length;
  const blocks = results.map((item) => {
    if (item.status === "ok") {
      return buildKaimeRaceBlock({
        race: item.result.race,
        oddsByCombination: item.result.oddsByCombination,
        plans: item.result.plans,
        anaMinCombinedOdds: item.result.anaMinCombinedOdds,
        honmeiMinCombinedOdds: item.result.honmeiMinCombinedOdds,
        honmeiMaxCombinedOdds: item.result.honmeiMaxCombinedOdds
      });
    }

    return [
      `【${item.race.placeName} ${item.race.raceNo}R】 買い目算出失敗`,
      `備考: ${item.error.message}`
    ].join("\n");
  });
