...
async function scrapeTarget(context, target) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    try {
      ...
      let baseResult = { ...target, matches: [], escapeRisk: calculateEscapeRisk([]) };
...
async function runWorkers(context, targets, config) {
  ...
  matchedResults.sort((left, right) => {
    if (right.escapeRisk.score !== left.escapeRisk.score) {
      return right.escapeRisk.score - left.escapeRisk.score;
    }
