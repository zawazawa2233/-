...
function formatRaceBlock(result) {
  const lines = [
    ...
    `逃げ危険度: ${result.escapeRisk.label} (${result.escapeRisk.score}) ${result.escapeRisk.summary}`,
    `URL: ${result.url}`,
    "一致条件:"
  ];
...
function buildEscapeRiskSummaryLine(results) {
  if (results.length === 0) {
    return null;
  }
  ...
  const highlighted = results
    .slice(0, 5)
    .map((result) => `${result.placeName} ${result.raceNo}R ${result.escapeRisk.label}${result.escapeRisk.score}`);
  const remainingCount = results.length - highlighted.length;

  return `逃げ危険度上位: ${highlighted.join(" / ")}${remainingCount > 0 ? ` / 他${remainingCount}レース` : ""}`;
}
...
export async function runDailyPick() {
  ...
  try {
    ...
    const summaryLines = [
      ...
      buildEscapeRiskSummaryLine(matchedResults),
